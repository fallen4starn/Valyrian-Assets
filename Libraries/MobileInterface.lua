...
local setThreadIdentity, getThreadIdentity do
    ...
    local success, setter = pcall(firstCallable,
    ...
    if success then
        setThreadIdentity = setter
    end

    local successGet, getter = pcall(firstCallable,
    ...
    if successGet then
        getThreadIdentity = getter
    end
end

local function withUIIdentity(callback, ...)
...
do
    ...
    Instance.new = function(className, parent)
        return withUIIdentity(function()
            local object = originalInstanceNew(className)
            if parent ~= nil then
